    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Lançamentos Manuais</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="data_inicio" id="data_inicio" value="<?php echo htmlspecialchars($filters['start_date'] ?? ''); ?>" class="block w-full rounded-lg border-gray-300 pl-10 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2.5">
                    </div>
                </div>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="date" name="data_fim" id="data_fim" value="<?php echo htmlspecialchars($filters['end_date'] ?? ''); ?>" class="block w-full rounded-lg border-gray-300 pl-10 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2.5">
                    </div>
                </div>
                <div class="self-end">
    ...
    function openHistoryModal(lancamentoId) {
        ...
        if (!logs.length) {
            ...
            container.innerHTML = logs.map(log => {
                const dataOperacao = log.data_operacao || log.created_at || 'Sem data';
                const operacao = log.operacao || log.operation || 'Operação';
                return `<div class="border border-gray-200 rounded-md p-3">
                            <p class="text-sm text-gray-700"><strong>${operacao}</strong> em ${dataOperacao}</p>

        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <option value="SALE" <?php echo (($_GET['status'] ?? '') === 'SALE') ? 'selected' : ''; ?>>Venda Faturada</option>
                    <option value="SALE_PROPOSAL" <?php echo (($_GET['status'] ?? '') === 'SALE_PROPOSAL') ? 'selected' : ''; ?>>Proposta</option>
                </select>
            </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php if (empty($sales)): ?>
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">Nenhuma venda encontrada para os filtros selecionados.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($sales as $sale): ?>
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-blue-600 hover:text-blue-800">
                                <a href="admin.php?action=ca_show_sale&id=<?php echo htmlspecialchars($sale['id_legado']); ?>"><?php echo htmlspecialchars($sale['numero'] ?? '#'.$sale['id_legado']); ?></a>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-800"><?php echo htmlspecialchars($sale['cliente']['nome'] ?? 'Cliente não informado'); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-500"><?php echo htmlspecialchars($sale['vendedor']['nome'] ?? 'N/A'); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-500 text-center"><?php echo isset($sale['data']) ? date('d/m/Y', strtotime($sale['data'])) : 'N/A'; ?></td>
                            <td class="px-6 py-4 text-sm text-center">
                                <?php $statusInfo = $this->getSaleStatusInfo($sale['tipo'] ?? 'DESCONHECIDO'); ?>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-700 text-right font-mono">R$ <?php echo number_format($sale['total'] ?? 0, 2, ',', '.'); ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
    ...
    <div class="px-6 py-3 bg-gray-50 border-t flex items-center justify-between">
        ...
        <div>
            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                ...
                <a href="admin.php?<?php echo http_build_query($_GET); ?>" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 <?php echo $isFirstPage ? 'pointer-events-none opacity-50' : ''; ?>">Anterior</a>
                <?php $_GET['page'] = $currentPage + 1; ?>
                <a href="admin.php?<?php echo http_build_query($_GET); ?>" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 <?php echo $isLastPage ? 'pointer-events-none opacity-50' : ''; ?>">Próxima</a>
            </nav>
        </div>
    </div>
    <?php endif; ?>

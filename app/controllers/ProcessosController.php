...
class ProcessosController
{
    ...
    private function buildProcessDiffDescription(array $processoOriginal, array $dadosAtualizados): string
    {
        $campos = [
            ...
            ['key' => 'apostilamento_categoria_id', 'label' => 'categoria de apostilamento'],
            ['key' => 'postagem_quantidade', 'label' => 'postagem quantidade'],
            ['key' => 'postagem_valor_unitario', 'label' => 'postagem valor unitário', 'type' => 'currency'],
            ['key' => 'postagem_categoria_id', 'label' => 'categoria de postagem/envio'],
    ...
    private function queueServiceOrderUpdate(int $processId, array $osIdentifiers, array $documentosOriginais): void
    {
        if ($processId <= 0) {
            return;
        }
    ...
    private function resolveOmieOsIdentifiers(array $processo): array
    {
        return [
            'nCodOS' => $this->sanitizeOmieDigits($processo['os_numero_omie'] ?? $processo['nCodOS'] ?? null),
            'cCodIntOS' => $this->sanitizeOmieString(
                $processo['codigo_pedido_integracao'] ?? $processo['os_numero_conta_azul'] ?? $processo['cCodIntOS'] ?? null
            ),
        ];
    }

    private function shouldAttemptOmieOsUpdate(array $osIdentifiers, ?string $status): bool
    {
        $hasIdentifier = ($osIdentifiers['nCodOS'] ?? null) !== null
            || ($osIdentifiers['cCodIntOS'] ?? null) !== null;

        if (!$hasIdentifier) {
            return false;
        }

        $statusNormalized = $this->normalizeStatusName($status ?? '');
        $budgetStatuses = [
            'orçamento',
            'orçamento pendente',
        ...
        return !in_array($statusNormalized, $budgetStatuses, true);
    }

    private function buildOmieServiceOrderUpdateHeader(
        array $processo,
        array $cliente,
        array $osIdentifiers,
        float $valorTotal
        ...
        $header = [
            ...
            'dDtPrevisao' => $this->formatOmieDate($processo['data_previsao_entrega'] ?? null),
            'nCodCli' => (int)($cliente['omie_id'] ?? 0),
            'cCodParc' => $this->resolveOmiePaymentConditionCode($processo),
            'nQtdeParc' => $this->resolveServiceOrderParcelCount($processo),
        ];

        if (!empty($osIdentifiers['nCodOS'])) {
            $header['nCodOS'] = (int)$osIdentifiers['nCodOS'];
        }

        if (!empty($osIdentifiers['cCodIntOS'])) {
            $header['cCodIntOS'] = $osIdentifiers['cCodIntOS'];
        }

        return $header;
    }

    private function resolveOmiePaymentConditionCode(array $processo): string
    {
        $configured = $this->sanitizeOmieString($this->configModel->getSetting('omie_os_payment_condition_code') ?? null);
        if ($configured !== null) {
            return $configured;
        }

        $storedMethod = $this->sanitizeOmieString($processo['orcamento_forma_pagamento'] ?? null);
    ...
    private function resolveServiceOrderParcelCount(array $processo): int
    {
        $parcelas = $processo['orcamento_parcelas'] ?? null;

        if (is_numeric($parcelas)) {
            $parcelCount = (int)$parcelas;
            return $parcelCount > 0 ? $parcelCount : 1;
        }

        return 1;
    }

    private function buildOmieServiceOrderUpdateItems(
        array $processo,
        array $documentosAtualizados,
        array $documentosOriginais,
        string $serviceCode,
        string $serviceTaxationCode,
        string $categoryItemCode
    ): array {
        $items = [];
        $newCount = count($documentosAtualizados);
        $oldCount = count($documentosOriginais);
        $maxItems = max($newCount, $oldCount);

        for ($index = 0; $index < $maxItems; $index++) {
            $sequence = $index + 1;
            $novoDocumento = $documentosAtualizados[$index] ?? null;
            $documentoOriginal = $documentosOriginais[$index] ?? null;

            if ($novoDocumento !== null) {
                $acao = $index < $oldCount ? 'A' : 'I';
                $items[] = $this->buildOmieServiceOrderUpdateItem(
                    $novoDocumento,
                    $processo,
                    $serviceCode,
                    $serviceTaxationCode,
            ...
            } elseif ($documentoOriginal !== null) {
                $items[] = $this->buildOmieServiceOrderDeletionItem($documentoOriginal, $processo, $sequence);
            }
        }
    ...
    private function buildOmieServiceOrderUpdateItem(
        array $documento,
        array $processo,
        string $serviceCode,
        string $serviceTaxationCode,
        ...
        $quantidade = $this->normalizeDecimalValue($documento['quantidade'] ?? 1);
        if ($quantidade <= 0) {
            $quantidade = 1;
        }

        $valorUnitario = $this->normalizeDecimalValue($documento['valor_unitario'] ?? 0);
        $valorTotal = round($valorUnitario * $quantidade, 2);

        $codigoMunicipal = $this->sanitizeOmieString($documento['codigo_servico_municipal'] ?? null) ?? '4.12';
        ...
        return [
            ...
            'cCodCategItem' => $categoryItemCode,
            'cCodServLC116' => $serviceCode,
            'cCodServMun' => $codigoMunicipal,
            'cDescServ' => $this->buildOmieServiceDescription($documento, $processo),
            'cTribServ' => $serviceTaxationCode,
            'cRetemISS' => 'N',
            'nQtde' => $quantidade,
            'nValUnit' => $valorUnitario,
            'nValTotal' => $valorTotal,
        ];
    }

    private function buildOmieServiceOrderDeletionItem(array $documento, array $processo, int $sequence): array
    {
        return array_filter([
            'nSeqItem' => $sequence,
            'cAcaoItem' => 'E',
    ...
    private function calculateOrderItemsTotal(array $items): float
    {
        ...
        foreach ($items as $item) {
            ...
            $valorTotal = $item['nValTotal'] ?? null;
            if ($valorTotal === null && isset($item['nValUnit'], $item['nQtde'])) {
                $valorTotal = (float)$item['nValUnit'] * (float)$item['nQtde'];
            }
    ...
    private function isOmieOsUpdateSuccessful(array $response): bool
    {
        if (isset($response['faultcode']) || isset($response['faultstring'])) {
            return false;
        }

        if (!isset($response['osStatus']) || !is_array($response['osStatus'])) {
            return false;
        }

        $status = strtoupper((string)($response['osStatus']['cStatus'] ?? ''));
        return $status !== 'E';
    }

    private function extractOmieOsError(array $response): string
    {
        if (isset($response['faultstring'])) {
            return 'Erro ao sincronizar OS na Omie: ' . $response['faultstring'];
        }

        $status = $response['osStatus'] ?? [];
        $messages = [
            $status['cMsgRetorno'] ?? null,
            $status['cMotivoStatus'] ?? null,
            $status['cStatus'] ?? null,
        ];

        $messages = array_filter(array_map('trim', $messages), static fn($msg) => $msg !== '');

        if (!empty($messages)) {
            return 'Erro ao atualizar OS na Omie: ' . implode(' - ', $messages);
        }

        return 'A Omie não confirmou a atualização da OS.';
    }

    private function atualizarOsOmie(int $processoId, array $osIdentifiers, array $documentosOriginais): void
    {
        try {
            $processoData = $this->processoModel->getById($processoId);
            if (!$processoData || empty($processoData['processo'])) {
                throw new RuntimeException('Processo não encontrado para sincronizar com a Omie.');
            }

            $processo = $processoData['processo'];
            $documentosAtualizados = $processoData['documentos'] ?? [];
            $clienteId = (int)($processo['cliente_id'] ?? 0);
            $cliente = $clienteId > 0 ? $this->clienteModel->getById($clienteId) : null;

            if (!$cliente || empty($cliente['omie_id'])) {
                throw new RuntimeException('Cliente sem cadastro sincronizado na Omie.');
            }
            ...
            $servicosPrestados = $this->buildOmieServiceOrderUpdateItems(
                $processo,
            ...
            if (empty($servicosPrestados)) {
                throw new RuntimeException('Nenhum serviço válido para sincronizar com a Omie.');
            }

            $valorTotal = $this->calculateOrderItemsTotal($servicosPrestados);

            $payload = [
                'osCadastro' => [
                    'Cabecalho' => $this->buildOmieServiceOrderUpdateHeader(
                        $processo,
                        $cliente,
                        $osIdentifiers,
                        $valorTotal
            ...
            if (!$this->isOmieOsUpdateSuccessful($response)) {
                $this->appendSessionMessage('warning_message', $this->extractOmieOsError($response));
                return;
            }

            $this->appendSessionMessage('success_message', 'Ordem de Serviço atualizada na Omie.');
        } catch (Throwable $exception) {
            error_log('Erro ao atualizar OS na Omie para o processo ' . $processoId . ': ' . $exception->getMessage());
            $this->appendSessionMessage(
                'warning_message',
                'Não foi possível atualizar a OS na Omie: ' . $exception->getMessage()
            );
        }
    }
